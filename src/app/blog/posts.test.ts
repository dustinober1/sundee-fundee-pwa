...
describe("blog content validation", () => {
  it("loads all blog posts with valid dates and interactive metadata", () => {
    const loadedPosts = loadPosts({ todayIso: "2026-05-15" });
    ...
    expect(
      loadedPosts.flatMap((post) => validateBlogPost(post, "2026-05-15")),
    ).toEqual([]);
  ...
  it("rejects a future publish date", () => {
    const [post] = loadPosts({ todayIso: "2026-05-15" });
    const invalidPost = {
      ...post,
    ...
    expect(validateBlogPost(invalidPost, "2026-05-15")).toContain(
      "publishedAt 2026-05-16 cannot be after 2026-05-15",
    );
  });

  it("keeps priority legacy articles substantial enough for search", () => {
    const postsBySlug = new Map(
      loadPosts({ todayIso: "2026-05-15" }).map((post) => [post.slug, post]),
    );
    const prioritySlugs = [
