...
| Programs | Mark program session complete | "Complete session" advances enrollment session index/week and logs a completed workout record. | `src/components/pwa/AppExperience.tsx` `src/lib/pwa/local-repositories.ts` | Completion creates a workout record with source `program`. |
