...
| Training | Workout execution session with timers, rest, substitutions, and rich detail views | iOS: workout execution / session flow (multiple views) | missing | Build dedicated session runner UI + richer model beyond single-set logging | iOS evidence: see native workout/session flow modules (research handoff); web evidence: current UI only supports quick set logging | High: core value prop; requires careful UX + data model | Web has no session-mode execution, substitutions, or timers today. |
...
| Sensors | HealthKit import of workouts, heart rate, HRV, etc. | iOS: HealthKit | web-impossible | No direct HealthKit; consider manual import / device integrations later | Constraint: HealthKit is Apple-only; Web cannot access it directly (R014 classification) | High: data gaps; parity expectations | Marked web-impossible only for direct HealthKit parity; replacement may exist via manual import or vendor APIs. |
